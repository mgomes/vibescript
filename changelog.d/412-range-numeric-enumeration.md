- **Added: Ruby-style range and numeric enumeration helpers.** Integers now
  ...
  `select`, `reject`) honor the memory quota as the result grows. `step`
  arguments must be nonzero (integers) and positive (ranges), `sum` errors on
  64-bit overflow, and the terminal value is detected before each increment so a
  span reaching the 64-bit bounds stops cleanly instead of wrapping.
