...
- Element writes to a local known to be `array<T>` — `items << v`,
  ...
  gradual.
- Entry writes to a local known to be `hash<K, V>` — `h[k] = v` and `store` —
  check the key against `K` and the value against `V` the same way. Writes to a local with a declared shape
