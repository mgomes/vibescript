...
func TestHoverIncludeExtendDirectiveIsContextual(t *testing.T) {
	...
	inMethod := "class C\n  def run\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, inMethod, 2, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method) = %q, must not serve mixin-removal docs", got)
	}
	classLocal := "class C\n  include = 1\n  include\nend\n"
	...
	numeric := "class C\n  include 1\nend\n"
	if got := hoverValue(t, numeric, 1, 2); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include 1) = %q, must not serve mixin-removal docs", got)
	}
	keywordArg := "class C\n  include true\nend\n"
	if got := hoverValue(t, keywordArg, 1, 2); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include true) = %q, must not serve mixin-removal docs", got)
	}
	selfArg := "class C\n  include self\nend\n"
	if got := hoverValue(t, selfArg, 1, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include self) = %q, want mixin-removal docs", got)
	}
	forLocal := "class C\n  def values\n    [1]\n  end\n  for include in values\n  end\n  include\nend\n"
	if got := hoverValue(t, forLocal, 6, 2); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after for) = %q, must not serve mixin-removal docs", got)
	}
	sameLine := "class C\n  include = 1; include\nend\n"
	if got := hoverValue(t, sameLine, 1, 15); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after same-line assignment) = %q, must not serve mixin-removal docs", got)
	}
	afterMethod := "class C\n  def run\n    1\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, afterMethod, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after method) = %q, want mixin-removal docs", got)
	}
	afterCommentedEnd := "class C\n  def run\n    1\n  end # run\n  include Naming\nend\n"
	if got := hoverValue(t, afterCommentedEnd, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after commented end) = %q, want mixin-removal docs", got)
	}
	nestedOnDef := "class C\n  def run; if true; end\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, nestedOnDef, 2, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include inside method after nested end) = %q, must not serve mixin-removal docs", got)
	}
	inClassIf := "class C\n  if true\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, inClassIf, 2, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in class-level if) = %q, must not serve mixin-removal docs", got)
	}
	endInString := "class C\n  def run(label = \"end\")\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, endInString, 2, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method with end in string) = %q, must not serve mixin-removal docs", got)
	}
	unindentedNested := "class C\n  def run\nif true\nend\n    include Naming\n  end\nend\n"
	...
	nestedClassMethod := "class Outer\n  class Inner\n    def run\n      include Naming\n    end\n  end\nend\n"
	if got := hoverValue(t, nestedClassMethod, 3, 6); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in nested class method) = %q, must not serve mixin-removal docs", got)
	}
	endHashLabel := "class C\n  def run\n    values = {end: 1}\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, endHashLabel, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after end hash label) = %q, must not serve mixin-removal docs", got)
	}
	afterWhileDo := "class C\n  def run\n    while false do\n      1\n    end\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, afterWhileDo, 6, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after while do) = %q, want mixin-removal docs", got)
	}
	endSymbol := "class C\n  def run\n    marker = :end\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, endSymbol, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after :end symbol) = %q, must not serve mixin-removal docs", got)
	}
	eachDoAfterWhile := "class C\n  def run\n    while false\n      values.each do\n        1\n      end\n    end\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, eachDoAfterWhile, 7, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after while/each do) = %q, must not serve mixin-removal docs", got)
	}
	memberEnd := "class C\n  def run\n    match.end(0)\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, memberEnd, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after match.end) = %q, must not serve mixin-removal docs", got)
	}
	percentEnd := "class C\n  def run\n    values = %w[end]\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, percentEnd, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after %%w[end]) = %q, must not serve mixin-removal docs", got)
	}
	sameLineAfterIf := "class C\n  if true; 1; end; include Naming\nend\n"
	if got := hoverValue(t, sameLineAfterIf, 1, 20); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after same-line if) = %q, want mixin-removal docs", got)
	}
	inCase := "class C\n  case 1\n  when 1\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, inCase, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in class-level case) = %q, must not serve mixin-removal docs", got)
	}
	multilineString := "class C\n  def run\n    s = \"\nend\n\"\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, multilineString, 5, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after multiline string) = %q, must not serve mixin-removal docs", got)
	}
	assignRHS := "class C\n  x = include Naming\nend\n"
	if got := hoverValue(t, assignRHS, 1, 6); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include on assignment RHS) = %q, must not serve mixin-removal docs", got)
	}
	assignIf := "class C\n  x = if true; include Naming; end\nend\n"
	if got := hoverValue(t, assignIf, 1, 16); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in assigned if) = %q, must not serve mixin-removal docs", got)
	}
	sameLineAssign := "class C\n  x = 1; include Naming\nend\n"
	if got := hoverValue(t, sameLineAssign, 1, 9); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after same-line assignment) = %q, want mixin-removal docs", got)
	}
	nestedPercent := "class C\n  def run\n    values = %w[[x] end]\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, nestedPercent, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after nested percent) = %q, must not serve mixin-removal docs", got)
	}
	...
	sameLineClassDef := "class C; def run\n  1\nend\ninclude Naming\nend\n"
	if got := hoverValue(t, sameLineClassDef, 3, 0); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after same-line class/def) = %q, want mixin-removal docs", got)
	}
	commandRegex := "class C\n  def run\n    match /end/\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, commandRegex, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after match /end/) = %q, must not serve mixin-removal docs", got)
	}
	inRange := "class C\n  x = 1..include(Naming)\nend\n"
	if got := hoverValue(t, inRange, 1, 9); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in range) = %q, must not serve mixin-removal docs", got)
	}
	assignedWhile := "class C\n  def run\n    result = while false\n      1\n    end\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, assignedWhile, 5, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after assigned while) = %q, must not serve mixin-removal docs", got)
	}
	sameLineWhile := "class C; def run; while false; 1; end; include Naming; end; end\n"
	if got := hoverValue(t, sameLineWhile, 0, 39); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after same-line while) = %q, must not serve mixin-removal docs", got)
	}
	inString := "class C\n  \"include Naming\"\nend\n"
	...
	assignedWhileBody := "class C\n  result = while false\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, assignedWhileBody, 2, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in assigned while) = %q, must not serve mixin-removal docs", got)
	}
	interpEnd := "class C\n  def run\n    s = \"#{\"end\"}\"\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, interpEnd, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interpolated end) = %q, must not serve mixin-removal docs", got)
	}
	commentInclude := "class C\n  # include Naming\nend\n"
	if got := hoverValue(t, commentInclude, 1, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in comment) = %q, must not serve mixin-removal docs", got)
	}
	regexClass := "class C\n  def run\n    pattern = /[a/end]/\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, regexClass, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after regex class) = %q, must not serve mixin-removal docs", got)
	}
	propertyName := "class C\n  property include\nend\n"
	if got := hoverValue(t, propertyName, 1, 11); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(property include) = %q, must not serve mixin-removal docs", got)
	}
	yieldCall := "class C\n  x = yield include(Naming)\nend\n"
	if got := hoverValue(t, yieldCall, 1, 12); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in yield) = %q, must not serve mixin-removal docs", got)
	}
	propertyThenDirective := "class C; property include; include; end\n"
	if got := hoverValue(t, propertyThenDirective, 0, 27); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after property include) = %q, want mixin-removal docs", got)
	}
	dotMember := "class C\n  obj.include()\nend\n"
	if got := hoverValue(t, dotMember, 1, 6); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(obj.include) = %q, must not serve mixin-removal docs", got)
	}
	percentInterp := "class C\n  def run\n    values = %W[#{\"]\"}]\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, percentInterp, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after percent interpolation) = %q, want mixin-removal docs", got)
	}
	blockComment := "class C\n=begin\ninclude Naming\n=end\nend\n"
	if got := hoverValue(t, blockComment, 2, 0); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in =begin) = %q, must not serve mixin-removal docs", got)
	}
	nestedSameLine := "class Outer; class Inner; include = 1; end; include; end\n"
	if got := hoverValue(t, nestedSameLine, 0, 45); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after nested class) = %q, want mixin-removal docs", got)
	}
	aliasName := "class C\n  alias include original\nend\n"
	if got := hoverValue(t, aliasName, 1, 8); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(alias include) = %q, must not serve mixin-removal docs", got)
	}
	blockCommentQuote := "class C\n=begin\n\"\n=end\n  include Naming\nend\n"
	if got := hoverValue(t, blockCommentQuote, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after =begin quote) = %q, want mixin-removal docs", got)
	}
	assignedBegin := "class C\n  result = begin\n    include Naming\n  rescue\n    nil\n  end\nend\n"
	if got := hoverValue(t, assignedBegin, 2, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in assigned begin) = %q, must not serve mixin-removal docs", got)
	}
	postfixStringIf := "class C\n  def run(ready)\n    \"x\" if ready\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, postfixStringIf, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after postfix string if) = %q, want mixin-removal docs", got)
	}
	unicodeComment := "class C\n  ππππ # include Naming\nend\n"
	if got := hoverValue(t, unicodeComment, 1, 10); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after unicode comment) = %q, must not serve mixin-removal docs", got)
	}
	multilinePercentInterp := "class C\n  def run\n    values = %W[#{ \n\"] end\"\n}]\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, multilinePercentInterp, 5, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after multiline percent interp) = %q, must not serve mixin-removal docs", got)
	}
	regexLeadingClose := "class C\n  def run\n    pattern = /[]/]end/\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, regexLeadingClose, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after regex leading ]) = %q, must not serve mixin-removal docs", got)
	}
	posixClass := "class C\n  def run\n    pattern = /[[:alpha:]/]end/\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, posixClass, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after posix class) = %q, must not serve mixin-removal docs", got)
	}
	interpRegex := "class C\n  def run\n    text = \"#{/\"/}\"\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, interpRegex, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp regex) = %q, must not serve mixin-removal docs", got)
	}
	unicodeMember := "class C\n  π.include()\nend\n"
	if got := hoverValue(t, unicodeMember, 1, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(π.include) = %q, must not serve mixin-removal docs", got)
	}
	percentInterpRegex := "class C\n  def run\n    values = %W[#{/\"/}]\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, percentInterpRegex, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after percent interp regex) = %q, must not serve mixin-removal docs", got)
	}
	interpDivision := "class C\n  def run\n    text = \"#{1 / 2}\"\n  end\n  include Naming\nend\n"
	...
	knownLocalDivision := "class C; def run; total = 4; total /2; end; include Naming; end\n"
	if got := hoverValue(t, knownLocalDivision, 0, 44); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after known-local /2) = %q, want mixin-removal docs", got)
	}
	interpRegexClassSlash := "class C\n  def run\n    text = \"#{/[a/\\\" end]/}\"\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpRegexClassSlash, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp regex class slash) = %q, want mixin-removal docs", got)
	}
	numericDivision := "class C; def run; 4 /2; end; include Naming; end\n"
	if got := hoverValue(t, numericDivision, 0, 29); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after 4 /2) = %q, want mixin-removal docs", got)
	}
	barePosixOpen := "class C\n  def run\n    text = /[[:/]/\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, barePosixOpen, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after bare POSIX opener) = %q, want mixin-removal docs", got)
	}
	interpPercentQuote := "class C\n  def run\n    text = \"#{%W[\"]}\"\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpPercentQuote, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp percent array) = %q, want mixin-removal docs", got)
	}
	paramDivision := "class C; def run(total); total /2; end; include Naming; end\n"
	if got := hoverValue(t, paramDivision, 0, 40); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after param /2) = %q, want mixin-removal docs", got)
	}
	multilineInterpPercent := "class C\n  def run\n    text = \"#{%W[\n\"]}\"\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, multilineInterpPercent, 5, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after multiline interp percent) = %q, want mixin-removal docs", got)
	}
	assignTarget := "class C; items[include()] = 1; end\n"
	if got := hoverValue(t, assignTarget, 0, 15); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in assignment target) = %q, must not serve mixin-removal docs", got)
	}
	leakedParamRegex := "class C; def run(total); 1; end; total /include Naming/; end\n"
	if got := hoverValue(t, leakedParamRegex, 0, 40); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in class-body regex) = %q, must not serve mixin-removal docs", got)
	}
	parenlessParam := "class C; def run total; total /2; end; include Naming; end\n"
	if got := hoverValue(t, parenlessParam, 0, 39); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after parenless param /2) = %q, want mixin-removal docs", got)
	}
	doBlockLocal := "class C; values.each do |total|; end; total /include Naming/; end\n"
	if got := hoverValue(t, doBlockLocal, 0, 45); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after do-block param) = %q, must not serve mixin-removal docs", got)
	}
	braceBlockLocal := "class C; values.each { |total| }; total /include Naming/; end\n"
	if got := hoverValue(t, braceBlockLocal, 0, 41); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after brace-block param) = %q, must not serve mixin-removal docs", got)
	}
	interpCommentQuote := "class C\n  def run\n    text = \"#{\n# \\\" ignored\n1\n}\"\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpCommentQuote, 7, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp comment quote) = %q, want mixin-removal docs", got)
	}
	percentInterpRegexSameLine := "class C; def run; values = %W[#{/\"/}]; end; include Naming; end\n"
	if got := hoverValue(t, percentInterpRegexSameLine, 0, 44); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after same-line percent interp regex) = %q, want mixin-removal docs", got)
	}
	implicitItDivision := "class C; def run; values.each do; it /2; end; end; include Naming; end\n"
	if got := hoverValue(t, implicitItDivision, 0, 51); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after implicit it /2) = %q, want mixin-removal docs", got)
	}
	braceAssignLocal := "class C; values.each { total = 1 }; total /include Naming/; end\n"
	if got := hoverValue(t, braceAssignLocal, 0, 43); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after brace assignment local) = %q, must not serve mixin-removal docs", got)
	}
	destructureLocals := "class C; def run; total, x = [4, 1]; total /2; end; include Naming; end\n"
	if got := hoverValue(t, destructureLocals, 0, 52); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after destructure /2) = %q, want mixin-removal docs", got)
	}
	indexAssignNotLocal := "class C; items[total] = 1; total /include Naming/; end\n"
	if got := hoverValue(t, indexAssignNotLocal, 0, 34); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after index assignment) = %q, must not serve mixin-removal docs", got)
	}
	interpHashComment := "class C\n  def run\n    text = \"#{\n#{ ignored\n1\n}\"\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpHashComment, 7, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp #{ comment) = %q, want mixin-removal docs", got)
	}
	parenCallBrace := "class C; values.each() { total = 1 }; total /include Naming/; end\n"
	if got := hoverValue(t, parenCallBrace, 0, 45); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after parenthesized call brace) = %q, must not serve mixin-removal docs", got)
	}
	classLocalDivision := "class C; total = 4; def run; total /2; end; include Naming; end\n"
	if got := hoverValue(t, classLocalDivision, 0, 44); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after class-local /2) = %q, want mixin-removal docs", got)
	}
	explicitBlockParam := "class C; def run; values.each do |x|; it /end/; end; include Naming; end\n"
	if got := hoverValue(t, explicitBlockParam, 0, 53); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after explicit block param) = %q, must not serve mixin-removal docs", got)
	}
	multilineClassLocal := "class C\n  total = 4\n  def run\n    total /2\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, multilineClassLocal, 5, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after multiline class-local /2) = %q, want mixin-removal docs", got)
	}
	nestedImplicitIt := "class C\n  def run\n    values.each do\n      items.each do |x|\n        it /2\n      end\n    end\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, nestedImplicitIt, 8, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after nested implicit it /2) = %q, want mixin-removal docs", got)
	}
	outerLocalIsolated := "total = 4; class C; def run; total /end/; include Naming; end; end\n"
	if got := hoverValue(t, outerLocalIsolated, 0, 42); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after outer local) = %q, must not serve mixin-removal docs", got)
	}
	typedParamDivision := "class C; def run(total: int); total /2; end; include Naming; end\n"
	if got := hoverValue(t, typedParamDivision, 0, 45); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after typed param /2) = %q, want mixin-removal docs", got)
	}
	paramDefault := "class C; def run(x = other); other /end/; include Naming; end; end\n"
	if got := hoverValue(t, paramDefault, 0, 42); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after param default) = %q, must not serve mixin-removal docs", got)
	}
	parenlessNewline := "class C\n  def run x\n    helper /end/\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, parenlessNewline, 3, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after parenless param newline) = %q, must not serve mixin-removal docs", got)
	}
	optKwDefault := "class C; def run(x: make + other); other /end/; include Naming; end; end\n"
	if got := hoverValue(t, optKwDefault, 0, 48); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after optional-keyword default) = %q, must not serve mixin-removal docs", got)
	}
	forTarget := "class C; def run; for total in [4]; total /2; end; end; include Naming; end\n"
	if got := hoverValue(t, forTarget, 0, 56); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after for-loop target /2) = %q, want mixin-removal docs", got)
	}
	rescueBind := "class C; def run; begin; 1; rescue => total; total /2; end; end; include Naming; end\n"
	if got := hoverValue(t, rescueBind, 0, 65); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after rescue binding /2) = %q, want mixin-removal docs", got)
	}
	methodRescue := "class C; def run; 1; rescue => total; total /2; end; include Naming; end\n"
	if got := hoverValue(t, methodRescue, 0, 53); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after method-level rescue /2) = %q, want mixin-removal docs", got)
	}
	unionParam := "class C; def run(x: int | nil); helper /end/; include Naming; end; end\n"
	if got := hoverValue(t, unionParam, 0, 46); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after union type param) = %q, must not serve mixin-removal docs", got)
	}
	rescueModifier := "class C; def run; value = risky rescue fallback; map = {\"x\" => helper}; helper /end/; include Naming; end; end\n"
	if got := hoverValue(t, rescueModifier, 0, 86); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after rescue modifier) = %q, must not serve mixin-removal docs", got)
	}
	divOp := "class C; def /(other); 1; end; include Naming; end\n"
	if got := hoverValue(t, divOp, 0, 31); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after def /) = %q, want mixin-removal docs", got)
	}
	lowerPercent := "class C; def run; values = %w[#{\"]}; end; include Naming; end\n"
	if got := hoverValue(t, lowerPercent, 0, 42); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after lowercase percent) = %q, want mixin-removal docs", got)
	}
	genericComma := "class C; def run(x: hash<string, int>); int /end/; include Naming; end; end\n"
	...
	ivarDivision := "class C; def run; @total /2; end; include Naming; end\n"
	if got := hoverValue(t, ivarDivision, 0, 34); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after ivar /2) = %q, want mixin-removal docs", got)
	}
	selfDivision := "class C; def run; self /2; end; include Naming; end\n"
	if got := hoverValue(t, selfDivision, 0, 32); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after self /2) = %q, want mixin-removal docs", got)
	}
	percentQ := "class C; def run; q = [2]; total %q[\"]\"]; end; include Naming; end\n"
	if got := hoverValue(t, percentQ, 0, 47); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after modulo %%q index) = %q, want mixin-removal docs", got)
	}
	indexAssign := "class C; def run; items[0] = 1; items /end/; include Naming; end; end\n"
	if got := hoverValue(t, indexAssign, 0, 45); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after indexed assignment) = %q, must not serve mixin-removal docs", got)
	}
	moduloW := "class C; def run; w = [0]; 1 %w[\"]\"]; end; include Naming; end\n"
	if got := hoverValue(t, moduloW, 0, 43); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after modulo %%w index) = %q, want mixin-removal docs", got)
	}
	compareDefault := "class C; def run(x = 1 < 2, total = 4); total /2; end; include Naming; end\n"
	if got := hoverValue(t, compareDefault, 0, 55); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after comparison in default) = %q, want mixin-removal docs", got)
	}
	callDivision := "class C; def run; helper() /2; end; include Naming; end\n"
	if got := hoverValue(t, callDivision, 0, 36); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after helper() /2) = %q, want mixin-removal docs", got)
	}
	interpCallDiv := "class C; def run; text = " + `"#{helper() /2}"` + "; end; include Naming; end\n"
	if got := hoverValue(t, interpCallDiv, 0, 48); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp helper() /2) = %q, want mixin-removal docs", got)
	}
	braceCallDiv := "class C; def run; helper {} /2; end; include Naming; end\n"
	if got := hoverValue(t, braceCallDiv, 0, 37); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after helper {} /2) = %q, want mixin-removal docs", got)
	}
	symbolArgDiv := "class C; def run; consume :x /2; end; include Naming; end\n"
	if got := hoverValue(t, symbolArgDiv, 0, 38); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after consume :x /2) = %q, want mixin-removal docs", got)
	}
	multilineTarget := "class C\n  def run\n    *rest, (m,\n     n,\n     o) = values\n    m /2\n  end\n  include Naming\nend\n"
	...
	interpBlockParam := "class C\n  def run\n    text = " + `"#{[4].map { |total| total /2 }}"` + "\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpBlockParam, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp block param /2) = %q, want mixin-removal docs", got)
	}
	interpImplicitIt := "class C\n  def run\n    text = " + `"#{[4].map { it /2 }}"` + "\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpImplicitIt, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp implicit it /2) = %q, want mixin-removal docs", got)
	}
	interpAssignLocal := "class C\n  def run\n    text = " + `"#{[4].map { total = 4; total /2 }}"` + "\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpAssignLocal, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp block assignment /2) = %q, want mixin-removal docs", got)
	}
	interpDestructure := "class C\n  def run\n    text = " + `"#{[4].map { total, x = [4, 1]; total /2 }}"` + "\n  end\n  include Naming\nend\n"
	...
	helperThenAssign := "class C\n  def run\n    helper\n    x = 1\n    helper /end/\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, helperThenAssign, 5, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in method after complete-line then assign) = %q, must not serve mixin-removal docs", got)
	}
	nestedDef := "class C\n  def outer\n    total = 4\n    def inner\n      total /end/\n    end\n    include Naming\n  end\nend\n"
	if got := hoverValue(t, nestedDef, 6, 4); strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include in outer method after nested def) = %q, must not serve mixin-removal docs", got)
	}
	interpNestedPercent := "class C\n  def run\n    text = " + `"#{%W[#{"]"}]}"` + "\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpNestedPercent, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after nested interp percent array) = %q, want mixin-removal docs", got)
	}
	percentModuloWA := "class C\n  def run\n    wa = 2\n    collect %wa\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, percentModuloWA, 5, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after collect %%wa modulo) = %q, want mixin-removal docs", got)
	}
	operatorSymbol := "class C; def run; consume :% /2; end; include Naming; end\n"
	if got := hoverValue(t, operatorSymbol, 0, strings.Index(operatorSymbol, "include")); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after operator symbol /2) = %q, want mixin-removal docs", got)
	}
	parenlessTypedPair := "class C\n  def run first: int, total: int\n    total /2\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, parenlessTypedPair, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after parenless typed params /2) = %q, want mixin-removal docs", got)
	}
	interpNestedIf := "class C\n  def run\n    text = " + `"#{[4].map { total = 4; if true; 1; end; total /2 }}"` + "\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpNestedIf, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp block nested if /2) = %q, want mixin-removal docs", got)
	}
	hashDivision := "class C; def run; x = {} /2; end; include Naming; end\n"
	if got := hoverValue(t, hashDivision, 0, strings.Index(hashDivision, "include")); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after hash {} /2) = %q, want mixin-removal docs", got)
	}
	interpWhileDo := "class C\n  def run\n    text = " + `"#{[4].map { total = 4; while false do; 1; end; total /2 }}"` + "\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, interpWhileDo, 4, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp while-do /2) = %q, want mixin-removal docs", got)
	}
	singleQuotedHash := "class C; def run; text = '#{\"}'; end; include Naming; end\n"
	if got := hoverValue(t, singleQuotedHash, 0, strings.Index(singleQuotedHash, "include")); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after single-quoted #{) = %q, want mixin-removal docs", got)
	}
	interpSymbolDiv := "class C; def run; text = " + `"#{consume :x /2}"` + "; end; include Naming; end\n"
	if got := hoverValue(t, interpSymbolDiv, 0, strings.Index(interpSymbolDiv, "include")); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after interp symbol /2) = %q, want mixin-removal docs", got)
	}
	bracketDestructure := "class C\n  def run\n    values.each do |[first: int, total: int]|\n      total /2\n    end\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, bracketDestructure, 6, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after bracket typed destructure /2) = %q, want mixin-removal docs", got)
	}
	unionBlockParam := "class C\n  def run\n    values.each do |value: int | string, total: int|\n      total /2\n    end\n  end\n  include Naming\nend\n"
	if got := hoverValue(t, unionBlockParam, 6, 2); !strings.Contains(got, "Removed mixin") {
		t.Fatalf("hover(include after union typed block param /2) = %q, want mixin-removal docs", got)
	}
}
