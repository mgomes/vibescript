...
func isUniversalMember(property string) bool {
	switch property {
	case "itself", "dup", "clone", "freeze", "frozen?", "nil?", "eql?", "equal?", "send", "public_send", "tap", "yield_self":
		return true
	default:
...
func isUniversalDataSafe(property string) bool {
	switch property {
	case "itself", "dup", "clone", "freeze", "frozen?", "nil?", "eql?", "equal?", "send", "public_send":
		return true
	default:
		return isUniversalPredicate(property)
	}
...
func newBoundEqualityPredicate(property string, cell *boundReceiver, compare func(Value, Value) bool) Value {
	...
	val := NewCapturingBuiltin(name, func(exec *Execution, _ Value, args []Value, kwargs map[string]Value, block Value) (Value, error) {
		...
		if exec != nil && stringLikeOperand(cell.value) && stringLikeOperand(args[0]) {
