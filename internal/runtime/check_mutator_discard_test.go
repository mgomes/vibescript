...
func TestDiscardedMutatorOnTemporaryIsReported(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "include? ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].include?(1) { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
		...
		{
			name:   "sample ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].sample { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "permutation ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].permutation { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "combination ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].combination { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "repeated_combination ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].repeated_combination { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "repeated_permutation ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].repeated_permutation { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "all-aborting ensure if is not an observation of the suffix",
			source: "def f(flag)\n  rows = [[1], [2]]\n  rows.each { |row| while true; row.push(0); begin; break; ensure; if flag; raise \"x\"; else; raise \"y\"; end; end; end; puts row }\nend\nf(true)\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "nested try else raise in ensure is not an observation of the suffix",
			source: "rows = [[1], [2]]\nrows.each { |row| while true; row.push(0); begin; break; ensure; begin; nil; rescue; nil; else; raise \"x\"; end; end; end; puts row }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "freeze ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].freeze { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "dup ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].dup { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "clone ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].clone { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "frozen? ignores its block so it is not an observation",
			source: "rows = [[1], [2]]\nrows.each { |row| row.push(0); [1].frozen? { puts row } }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
		...
		{
			name:   "raise before next is not an observation of the loop condition",
			source: "rows = [[1], [2]]\nrows.each { |row| while row.length < 3; row.push(0); raise \"x\"; next; end }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "infinite ensure while raise is not an observation of the suffix",
			source: "rows = [[1], [2]]\nrows.each { |row| while true; row.push(0); begin; break; ensure; while true; raise \"x\"; end; end; end; puts row }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
		...
		{
			name:   "true elsif raise shadows else so suffix is unreachable",
			source: "def f(flag)\n  rows = [[1], [2]]\n  rows.each { |row| row.push(0); if flag; raise \"x\"; elsif true; raise \"y\"; else; nil; end; puts row }\nend\nf(true)\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
		...
		{
			name:   "ensure next after break is not an observation of the suffix",
			source: "rows = [[1], [2]]\nrows.each { |row| while true; row.push(0); begin; break; ensure; next; end; end; puts row }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
		...
		{
			name:   "single-element for does not observe via a back edge",
			source: "rows = [[1], [2]]\nrows.each { |row| for x in [row]; row.push(0); end }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "first matching ensure rescue raise is not a loop exit",
			source: "rows = [[1], [2]]\nrows.each { |row| while true; row.push(0); begin; break; ensure; begin; raise TypeError, \"t\"; rescue TypeError; raise \"x\"; rescue; nil; end; end; end; puts row }\nputs \"done\"",
			want:   "mutating block parameter row",
		},
		{
			name:   "raising for over nonempty alias does not observe the suffix",
...
func TestLegitimateMutationsAreNotReported(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "empty hash replace does not write a temporary",
			source: "{}.replace({})\nputs \"done\"",
		},
		{
		...
		{
			name:   "nested if break still observes statements after the loop",
			source: "rows = [[1], [2]]\nrows.each { |row| while true; row.push(0); if true; break; end; end; puts row }\nputs \"done\"",
		},
		{
		...
		{
			name:   "ensure while with nested try break observes the suffix",
			source: "rows = [[1], [2]]\nrows.each { |row| while true; row.push(0); begin; break; ensure; while true; begin; break; end; end; end; end; puts row }\nputs \"done\"",
		},
		{
