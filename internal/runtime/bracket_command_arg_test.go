...
// `puts[1]` still fails at runtime (now at the value reference, since a
// method is not a value) instead of becoming a call.
func TestBracketFlushAgainstBuiltinStaysIndexing(t *testing.T) {
    ...
    `)
	requireCallErrorContains(t, script, "run", nil, CallOptions{}, "cannot be used as a value")
}
