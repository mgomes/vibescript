...
//
// The elements a builtin already captured must keep their values. Array drivers
// snapshot receiver.Array() before the first yield and walk that header while
// the block runs, which is the iteration convention TestArrayMutationDuringIteration
// pins. Zeroing a slot the driver has not reached yet hands the block a nil that
// was never in the array: `a.each { a.pop }` over [1, 2, 3] yielded 1, 2, nil.
// Copying the survivors onto a fresh backing fixes that instead, by leaving the
...
// builtin frame is holding, and zeroes in place the rest of the time. The
// common shapes settle out as:
//
//   - `while a.size > 0; a.pop; end` -- no enclosing frame holds the backing,
//     so every pop zeroes in place and the loop stays linear.
//
//   - `a.each { a.pop }` -- the first pop copies, because each is holding the
//     header it is still walking. That copy is the receiver's new backing,
//     which each is not walking, so every later pop zeroes in place and the
//     loop stays linear.
//
...
// heldArrayBacking is one builtin frame's claim on an array element backing:
// the identity of the backing the frame was entered with, and the frame's own
// builtin depth. The depth is what lets a shrink tell an enclosing frame's
...
// holdArrayBacking records the element backing of an array receiver for the
// builtin frame the dispatcher is entering, and reports whether it recorded
// anything. The claim is taken at entry because that is where a driver captures
// the header it will walk: nothing has run in between that could have moved the
// receiver onto a different backing.
...
// arrayBackingHeldByCaller reports whether a builtin frame enclosing the
// running one is holding values' backing, and so may still read slots this
// frame is about to vacate. The running frame's own claim, recorded by its
// dispatch, sits at the current builtin depth and is not an enclosing one.
