...
// nothing is left on the execution for a later driver to be charged for. Making
// these the only two settlement points is deliberate -- as a per-driver
// convention it was simply omitted by every driver added after the first two,
// and the omission is invisible until a script pairs a large reachable graph
// with a low step quota.
//
