...
// The results map_with_index has produced so far live in a Go-local slice, so
// the block that allocates a large temporary was measured against a graph
// missing all of them. The 29 retained 20KB results and the final 600KB
// temporary each fit the 1MB quota alone, and the temporary is discarded before
// the post-call check, so only walking the retained output rejects the peak.
func TestHashMapWithIndexWalksRetainedOutputDuringBlockCalls(t *testing.T) {
	t.Parallel()

	script := compileScriptWithConfig(t, Config{StepQuota: Unlimited, MemoryQuotaBytes: 1024 * 1024}, `
    def run(h)
      h.map_with_index { |pair, i|
        if i == 29
          ("y" * 600000).length
        else
          "x" * 20000
    ...
    `)
	if _, err := script.Call(context.Background(), "run", []Value{hashOfIndexedInts(30)}, CallOptions{}); err == nil {
		t.Fatalf("retained map_with_index output plus an in-block temporary exceeded the quota but was accepted")
	}
}

// transform_values has the same shape: every transformed value stays in the
// Go-local result map until the builtin returns, so a block allocating a large
// temporary saw none of them.
func TestHashTransformValuesWalksRetainedOutputDuringBlockCalls(t *testing.T) {
	t.Parallel()
    ...
    `)
	if _, err := script.Call(context.Background(), "run", []Value{hashOfIndexedInts(30)}, CallOptions{}); err == nil {
		t.Fatalf("retained transform_values output plus an in-block temporary exceeded the quota but was accepted")
	}
}

// Both drivers must still answer the ordinary case, so the registration is
// released when they return and costs nothing it should not.
func TestHashMapWithIndexAndTransformValuesDoNotOverCharge(t *testing.T) {
	t.Parallel()

	sources := map[string]string{
		"map_with_index":   "def run(h)\n  h.map_with_index { |pair, i| \"x\" * 20000 }.length\nend",
		"transform_values": "def run(h)\n  h.transform_values { |v| \"x\" * 20000 }.length\nend",
	}

	for name, src := range sources {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			script := compileScriptWithConfig(t, Config{StepQuota: Unlimited, MemoryQuotaBytes: 8 << 20}, src)
			got, err := script.Call(context.Background(), "run", []Value{hashOfIndexedInts(30)}, CallOptions{})
			if err != nil {
				t.Fatalf("a 30-entry transform that fits was rejected: %v", err)
			}
			if got.Int() != 30 {
				t.Fatalf("result has %d elements, want 30", got.Int())
			}
		})
	}
}
