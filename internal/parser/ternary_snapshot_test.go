...
func TestParserSnapshotRestoreTernaryStack(t *testing.T) {
	...
	p.l.ternaryStack = append(p.l.ternaryStack, 7)
	p.l.bracketDepth = 3

	p.restore(saved)

	if diff := cmp.Diff([]int{0, 1}, p.l.ternaryStack); diff != "" {
	...
	p.l.ternaryStack = append(p.l.ternaryStack, 9)
	if diff := cmp.Diff([]int{0, 1}, saved.lexer.ternaryStack); diff != "" {
