...
- **Stdlib input guards:** JSON and Regex helpers enforce fixed caps — 1 MiB for `JSON.parse` input and `JSON.stringify` output, 10,000 nested JSON containers, 1 MiB for regex text/replacements/output, and 16 KiB for regex patterns. The canonical values live in `internal/runtime/limits.go`; see [docs/stdlib_core_utilities.md](docs/stdlib_core_utilities.md) for details.
