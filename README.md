...
- **Memory quota:** `Config.MemoryQuotaBytes` limits interpreter allocations (default 64 KiB). Exceeding the limit raises a runtime error instead of consuming host memory. An unlimited quota (`vibes.Unlimited`) skips the reachable-graph accounting entirely.
- **Quota profiles:** For coherent step/memory/recursion bundles, use the named profiles instead of setting each field by hand — `vibes.ProfileLow` (1M steps / 16 MiB / 256), `ProfileMedium` (20M / 128 MiB / 1,000), `ProfileHigh` (200M / 512 MiB / 4,000), and `ProfileXHigh` (unlimited / unlimited / 10,000). Apply one with `ProfileHigh.ApplyTo(&cfg)`, or look one up by name with `vibes.QuotaProfileByName`. The `vibes` CLI selects them via `-profile` and defaults to `xhigh` (it runs your own scripts, so it is not a sandbox); see [docs/tooling.md](docs/tooling.md#quota-profiles).
